       ...
       mysql->options.protocol == MYSQL_PROTOCOL_TCP))
  {
    ...
    */
    ...
    for (t_res= res_lst; t_res; t_res= t_res->ai_next)
    {
      ...
      /*
      Set SO_ECVBUF if this option is enabled
      */
      if (net->receive_buffer_size &&
        setsockopt(net->vio->mysql_socket.fd, SOL_SOCKET, SO_RCVBUF,
        (const char*)&net->receive_buffer_size,
        sizeof(net->receive_buffer_size)) == -1)
        DBUG_PRINT("error", ("Failed to set SO_RCVBUF with (error: %s).",
        strerror(errno)));

      if (net->tcp_no_delay &&
        setsockopt(net->vio->mysql_socket.fd, IPPROTO_TCP, TCP_NODELAY,
        (const char*)&net->tcp_no_delay,
        sizeof(net->tcp_no_delay)) == -1)
        DBUG_PRINT("error", ("Failed to set TCP_NODELAY with (error: %s).",
          strerror(errno)));
...
mysql_options(MYSQL *mysql,enum mysql_option option, const void *arg)
{
  ...
  case MYSQL_OPT_TCP_NO_DELAY:
    mysql->net.tcp_no_delay = *(uint*)arg;
    break;
